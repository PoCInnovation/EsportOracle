...
import {EsportOracleRequester} from "../src/esportOracleRequester.sol";
import {EsportOracleTypes} from "../src/esportOracleTypes.sol";
import {MockBettingClient} from "../src/MockBettingClient.sol";

contract OracleE2E is Script {
    ...
    function run() external {
        ...
        EsportOracleRequester oracle = new EsportOracleRequester();
        ...
        vm.startBroadcast(pk1);
        MockBettingClient client = new MockBettingClient(address(oracle));
        console.log("Client:", address(client));
        client.receiveMatch{value: 0.0001 ether}(42);
        vm.stopBroadcast();
        ...
        require(client.lastMatchId() == 42 && client.lastWinnerId() == 7, "client not delivered");
