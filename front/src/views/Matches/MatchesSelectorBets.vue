...
const specificMatch = computed(() => {
  if (!matchesWithActiveBets.value.length) return null
  const preferredTiers = new Set(['s', 'a'])
  const tierMatch = matchesWithActiveBets.value.find(match => preferredTiers.has(match.tournament?.tier?.toLowerCase() ?? ''))
  return tierMatch ?? matchesWithActiveBets.value[0]
})
